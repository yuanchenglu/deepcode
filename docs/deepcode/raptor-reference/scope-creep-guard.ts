...
// Effect 框架核心：Effect 用于效应管理，Layer 用于依赖注入，Ref 用于可变状态，Schema 用于数据校验
 ...
 * ScopeCreepGuard 服务
 *
 * 作为 DeepCode 管线中的一个 Effect Service，提供范围蔓延防护能力。
 * 所有状态通过 Ref 管理，纯内存操作，线程安全（Effect 保证）。
 */
export class Service extends Context.Service<Service, Interface>()("@opencode/v2/DeepCode/ScopeCreepGuard") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
     ...
     *
     * @param allowedFiles  - 计划中允许修改的文件 glob 列表
     * @param readonlyFiles - 允许读取但不修改的文件 glob 列表，默认为空
     * @returns Effect 效应，设置成功后无返回值
     */
     ...
     * 3. 通配符匹配：如 "src/*斜杠/test.ts" 转为正则 ^src/.*/test.ts$ 匹配
     * 4. 精确匹配：无通配符时要求完全相等或是该路径下的子路径
     *
     * @param path     - 待检查的文件路径
     * @param patterns - glob 模式列表
     * @returns true 表示路径匹配任一模式，false 表示不匹配
     */
     ...
     * @returns 检查结果对象：
     *   - allowed: 是否直接允许调用
     *   - reason: 不允许时的原因说明（可展示给用户）
     *   - needsApproval: 是否需要用户确认
     *   - dependencyType: 若不允许，建议的依赖分类
     */
     ...
     *
     * @param path         - 涉及的文件路径
     * @param type         - 依赖分类（required/optional/unrelated）
     * @param reason       - 修改原因/意图描述
     * @param userApproved - 用户是否已批准（required 类型忽略此参数，视为自动批准）
     * @returns Effect 效应，更新成功后无返回值
     */
        ...
        if (type === "required") {
          ...
          yield* Ref.update(scopeRef, (s) => ({
            ...
            deferredFindings: [...s.deferredFindings, {
              description: reason,
              location: path,
              severity: "medium", // TODO: 可根据信号词强度动态调整 severity
            }],
          }))
        }
     ...
     *
     * 检测模式（正则）：
     * - 中文："顺便...", "既然都...", "也可以/应该/需要...改/修/加/处理/优化/重构"
     * - 英文："while/since I'm at it...", "should/could/might as well also/fix/clean...",
     *         "in addition to..."
     *
     * @param text - 待检测的文本（通常是 assistant 的回复消息）
     * @returns 检测结果：
     *   - hasSignal: 是否检测到信号
     *   - signals:  匹配到的信号短语列表（截断至120字符，去重）
     */
    const detectScopeCreepSignals = (text: string): {
      hasSignal: boolean
     ...
     * 获取延迟处理的问题列表
     *
     * 任务结束时调用，将当前任务中发现但未处理的无关问题返回给调用方，
     * 由调用方格式化后报告给用户（可作为"额外发现"或"后续建议"呈现）。
     *
     * @returns Effect 效应，成功时返回 deferredFindings 列表
     */
     ...
     * 获取当前 Scope 快照
     *
     * 返回当前已批准范围的完整快照，用于调试、日志和状态展示。
     *
     * @returns Effect 效应，成功时返回 ApprovedScope 对象
     */
     ...
     * 获取超范围尝试的历史记录
     *
     * 返回所有 scope violation 记录，用于审计分析：
     * - 哪些类型的超范围最常见？
     * - 用户批准率是多少？
     * - 初始 scope 设置是否过于保守？
     *
     * @returns Effect 效应，成功时返回只读的 violation 记录数组
     */
     ...
     * 重置服务状态
     *
     * 在新任务开始前调用，清空所有 scope、violation 和 turn 计数，
     * 将服务恢复到初始状态。
     *
     * @returns Effect 效应，重置成功后无返回值
     */
    const reset = (): Effect.Effect<void> =>
      Ref.set(scopeRef, { allowedFiles: [], readonlyFiles: [], approvedOptionals: [], deferredFindings: [] }).pipe(
        Effect.zipRight(Ref.set(violationsRef, [])), // 清空违规记录
        Effect.zipRight(Ref.set(turnRef, 0)),         // 重置轮次计数
      )
 ...
 * 无外部服务依赖（deps 为空），使用默认 Layer。
 */
export const node = makeLocationNode({
  name: "deepcode-scope-creep-guard", // 节点唯一标识
  service: Service,                    // 服务类
  layer: layer,              // 默认依赖注入层
  deps: [],                            // 本服务不依赖其他 DeepCode 节点
})
