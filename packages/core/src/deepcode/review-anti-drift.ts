...
// Effect生态核心依赖：Context（DI token）、Effect（副作用管理）、Layer（依赖注入）、Ref（可变状态引用）
...
export interface ReviewStats {
  /** 已完成的 tool call 轮次（K 值） */
  readonly K: number
  /** 当前硬约束数量（P 值） */
  readonly P: number
  /** 计算出的当前审查档位 */
  readonly currentTier: ReviewTier
}
...
export interface Interface {
  /** 计算审查深度 f(K,P)（纯函数，可独立调用） */
  readonly computeTier: (K: number, P: number) => ReviewTier
   ...
   */
  readonly recordToolCall: (constraintCount: number) => Effect.Effect<RecordToolCallResult>
   ...
   * @returns 审查prompt字符串，档位0返回空字符串
   */
  readonly buildReviewPrompt: (tier: ReviewTier, currentStep?: string) => string

  /** 记录审查结果（更新lastReviewTier并追加审查记录） */
  readonly recordReview: (result: ReviewResult) => Effect.Effect<void>
   ...
   */
  readonly getReinjectionContent: (
    objective: string,
  ...
  /** 获取最近的审查结果列表（最多20条） */
  readonly getRecentReviews: () => Effect.Effect<readonly ReviewResult[]>

  /** 重置状态（新任务开始时调用） */
  readonly reset: () => Effect.Effect<void>

  /** 获取当前状态统计（K、P、currentTier） */
  readonly getStats: () => Effect.Effect<ReviewStats>
}

/** DI token — 从 旧API 迁移到 Context.Service */
export class Service extends Context.Service<Service, Interface>()(
  "@opencode/v2/DeepCodeReviewAntiDrift",
) {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const stateRef = yield* Ref.make<ReviewState>({
    ...
    const recordToolCall = (
      constraintCount: number,
    ): Effect.Effect<RecordToolCallResult> =>
...
这是对抗HCA注意力128x稀释的强制回拉机制，不是建议。`
    ...
    /** 获取当前状态统计 */
    const getStats = (): Effect.Effect<ReviewStats> =>
 ...
 * name用于服务发现和日志追踪，layer为Layer.effect构造的实现层，
