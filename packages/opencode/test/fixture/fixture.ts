...
export async function tmpdir<T>(options?: TmpDirOptions<T>) {
  const dirpath = sanitizePath(path.join(os.tmpdir(), "deepcode-test-" + Math.random().toString(36).slice(2)))
  ...
  if (options?.config) {
    await Bun.write(
      path.join(dirpath, "deepcode.json"),
      JSON.stringify({
