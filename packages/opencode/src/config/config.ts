...
const configFiles = [...Product.config.files]
...
function globalConfigFile() {
  const candidates = configFiles.toReversed().map((file) => path.join(Global.Path.config, file))
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        // Keep the upstream schema URL until a verified DeepCode schema endpoint exists.
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              await fsNode.writeFile(path.join(Global.Path.config, Product.config.files[0]), JSON.stringify(result, null, 2))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "DEEPCODE_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            // This upstream protocol path is retained as a network compatibility contract, not as product identity.
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files(Product.config.basename, ctx.directory, ctx.worktree).pipe(
            Effect.orDie,
          )) {
        ...
        if (Flag.OPENCODE_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from DEEPCODE_CONFIG_DIR", { path: Flag.OPENCODE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(Product.config.directory) || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const file of configFiles) {
          ...
          // Auto-discovered plugins under `.deepcode/plugin(s)` are already local files, so ConfigPlugin.load
        ...
        if (Flag.OPENCODE_CONFIG_CONTENT) {
          const source = "DEEPCODE_CONFIG_CONTENT"
          const next = yield* loadConfig(Flag.OPENCODE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from DEEPCODE_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["DEEPCODE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("DEEPCODE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.OPENCODE_PERMISSION) {
          try {
            ...
            yield* Effect.logWarning("DEEPCODE_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
    ...
    const update = Effect.fn("Config.update")(function* (config: Info) {
      ...
      const file = path.join(dir, Product.config.files[0])
