...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === Product.config.directory
        ? path.join(source_dir, "themes")
        : path.join(source_dir, Product.config.directory, "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, Product.config.directory, "tui.json"),
  }
}
